...
//! glyphsWithAlternates 扁平列表：一行 = 一个 alternate 条目
class AlternatesModel : public QAbstractListModel, public muse::Injectable, public muse::async::Asyncable
{
    Q_OBJECT
...
public:
    explicit AlternatesModel(QObject* parent = nullptr);
    ...
    Q_INVOKABLE void addAlternate(const QString& baseName, const QString& altName);
    Q_INVOKABLE void removeRowAt(int row);
    Q_INVOKABLE void setBaseName(int row, const QString& name);
    Q_INVOKABLE void setAltName(int row, const QString& name);
...
private:
    ...
    QList<int> m_visible; // indices into m_allItems
