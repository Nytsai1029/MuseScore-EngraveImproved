...
void TLayout::layoutGraceNotesGroup(GraceNotesGroup* item, LayoutContext& ctx)
{
    ...
    for (size_t i = item->size() - 1; i != muse::nidx; --i) {
        ...
        // Adjust spacing for cross-beam situations (unless the cross-staff beam spacing offset is disabled)
        if (i < item->size() - 1 && grace->stem() && ctx.conf().styleB(Sid::crossStaffBeamSpacingOffset)) {
